...
PinStats는 Windows 10/11에서 실시간 PC 자원 사용량을 표시하는 가벼운 앱입니다. CPU, GPU, 메모리, 네트워크, 스토리지, 배터리 정보를 표시하며 트레이 아이콘에 퍼센트를 표시하는 앱은 많지만 Windows 11 작업표시줄에 위젯을 직접 고정할 수 있는 앱은 거의 없습니다. PinStats는 그 위젯을 중심으로 실시간 숫자 트레이 아이콘과 작은 팝업, 원하는 모니터에 띄우는 풀스크린 하드웨어 모니터 대시보드까지 담았습니다.

Windows 11 작업표시줄 위젯은 사용량 링과 속도 수치를 항상 띄워 둡니다. 트레이 아이콘은 0.25초마다 현재 CPU 또는 GPU 사용량 숫자를 표시합니다. 좌클릭으로 작은 팝업을, 우클릭으로 선택한 모니터에 풀스크린 대시보드를 띄웁니다.
...
- **풍부한 센서 정보:** CPU, GPU, 메모리(물리 + 가상), 네트워크, 스토리지, 배터리(수명 상태 포함)의 사용량·온도·전력 소비와 마더보드 이름을 표시합니다.
...
트레이 아이콘에 퍼센트를 띄우는 앱은 많습니다. PinStats는 Windows 11 작업표시줄에 위젯까지 직접 고정합니다. 다른 시스템 모니터에서는 보기 드문 방식입니다. 위젯은 작업하는 동안 계속 떠 있어 CPU, GPU, 메모리, 네트워크, 스토리지 상태를 따로 창을 열지 않아도 한눈에 읽을 수 있습니다. 작은 팝업과 원하는 모니터에 띄우는 테두리 없는 풀스크린 대시보드가 나머지를 채워줍니다.
...
- **마더보드** — 모델명.
- **ARM64 SoC 전력** — ARM64 기기에서 CPU 패키지, GPU, SoC, 시스템 전력을 추가로 표시합니다.
